...
recurringRoutes.post("/:id/process", async (c) => {
  ...
  const recurring = await db.recurringTransaction.findFirst({ where: { id, userId, isActive: true } })
  if (!recurring) return c.json({ error: "Recurring transaction not found" }, 404)

  const accounts = await db.account.findMany({ where: { userId, isActive: true }, take: 1 })
  if (!accounts.length) return c.json({ error: "No active accounts found" }, 422)
  ...
  const tx = await db.$transaction(async (prisma) => {
    ...
    const balanceDelta = recurring.type === "INCOME" ? Number(recurring.amount) : -Number(recurring.amount)
    await prisma.account.update({ where: { id: accounts[0].id }, data: { balance: { increment: balanceDelta } } })
...
recurringRoutes.delete("/:id", async (c) => {
  ...
  await db.recurringTransaction.update({ where: { id }, data: { isActive: false } })
