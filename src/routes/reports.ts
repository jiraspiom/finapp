...
reportRoutes.get("/monthly", async (c) => {
  ...
  return c.json({ year, monthly, totals: { ...totals, net: totals.income - totals.expenses } })
})
...
reportRoutes.get("/by-category", async (c) => {
  ...
  const byCategory = transactions.reduce<Record<string, { category: unknown; total: number; count: number }>>((acc, t) => {
  ...
  const result = Object.values(byCategory)
    .map(item => ({ ...item, percentage: grandTotal > 0 ? Math.round((item.total / grandTotal) * 100) : 0 }))
...
reportRoutes.get("/cash-flow", async (c) => {
  ...
  for (let i = months - 1; i >= 0; i--) {
    ...
    const monthTx = transactions.filter(t => dayjs(t.date).month() === d.month() && dayjs(t.date).year() === d.year())
    const income = monthTx.filter(t => t.type === "INCOME").reduce((s, t) => s + Number(t.amount), 0)
    const expenses = monthTx.filter(t => t.type === "EXPENSE").reduce((s, t) => s + Number(t.amount), 0)
...
reportRoutes.get("/net-worth", async (c) => {
  ...
  const liabilities = accounts.filter(a => a.type === "CREDIT_CARD" && Number(a.balance) < 0)

  const totalAssets = assets.reduce((s, a) => s + Math.max(0, Number(a.balance)), 0)
  const totalLiabilities = Math.abs(liabilities.reduce((s, a) => s + Math.min(0, Number(a.balance)), 0))
